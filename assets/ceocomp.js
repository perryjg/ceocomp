		...
		function(data) {
			var IncomeModel = Backbone.Model.extend({
				initialize: function() {
					...
					this.on( 'change:wage', function() {
						this.set('income', (this.get('wage') * 40 * 52).toFixed(0) );
						console.log("Wage changed");
						ceoList.reset(data);
					});
				}
			});
			...
			var WageView = Backbone.View.extend({
				id: 'wage_entry',
				template: _.template( 'Or, enter your <b>hourly</b> wage: ' + $("#wage-tmplt").html() ),
				events: {
					'keypress .inpt' : 'enterWage',
					'click .btn'     : 'updateWage'
				},
				render: function() {
					var attributes = this.model.toJSON();
					this.$el.html( this.template( this.model.toJSON() ) );
				...
				updateWage: function(e) {
					console.log(e);
					this.model.set({ 'wage': this.$(".inpt").val() });
				},
				enterWage: function(e) {
					if ( e.keyCode ===13 ) {
						console.log(e.keyCode);
						this.model.set({ 'wage': this.$(".inpt").val() });
					}
			...
			wageView = new WageView({ model: incomeModel });
			wageView.render();
			$("div#input").append( wageView.el );
		}
	);
});
